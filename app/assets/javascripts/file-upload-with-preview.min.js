! function(A, g) {
  "object" ==
  typeof exports && "undefined" != typeof module ? module.exports = g() : "function" == typeof define && define.amd ? define(g) : A.FileUploadWithPreview = g()
}(this, function() {
  "use strict";
  ...
  var g = function() {
    ...
    return function(g, Q, B) {
      return Q && A(g.prototype, Q), B && A(g, B), g
    }
  }();

  return function() {
    function Q(g) {
      if (A(this, Q), !g) throw new Error("No uploadId found. You must initialize file-upload-with-preview with a unique uploadId.");
      if (this.uploadId = g, this.cachedFileArray = [], this.el = document.querySelector('.custom-file-container[data-upload-id="' + this.uploadId + '"]'), !this.el) throw new Error("Could not find a `custom-file-container` with the id of: " + this.uploadId);
    ...
    return g(Q, [{
        ...
        value: function() {
          var A = this,
            g = this;
          this.imagePreview.style.backgroundImage = 'url("' + this.baseImage + '")', this.input.addEventListener("change", function() {
            var A = this,
              Q = this.files.length;
            if (0 !== Q) {
              g.cachedFileArray = [];
              for (var B = 0; B < this.files.length; B++) ! function(B) {
                var I = A.files[B];
                g.cachedFileArray.push(I);
                var E = new FileReader;
                E.readAsDataURL(I), E.onload = function(A) {
                  if (Q > 1) return g.inputLabel.innerHTML = Q + " files selected", void(g.imagePreview.style.backgroundImage = 'url("' + g.successMultiple + '")');
                  g.inputLabel.innerHTML = I.name, I.type.match("image/png") || I.type.match("image/jpeg") ? g.imagePreview.style.backgroundImage = 'url("' + E.result + '")' : I.type.match("application/pdf") ? g.imagePreview.style.backgroundImage = 'url("' + g.successPdf + '")' : I.type.match("video/*") ? g.imagePreview.style.backgroundImage = 'url("' + g.successVideo + '")' : g.imagePreview.style.backgroundImage = 'url("' + g.successFileAlt + '")'
                }
              }(B)
            }
        ...
        value: function() {
          this.input.value = "", this.inputLabel.innerHTML = "", this.imagePreview.style.backgroundImage = 'url("' + this.baseImage + '")', this.cachedFileArray = []
        }
      }
